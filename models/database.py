...
DB_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "legal_tort.db"
)
...
def init_db():
    ...
    with get_db() as conn:
        ...
        return {"status": "ok", "total_judgments": count}
...
def get_db_stats() -> dict:
    ...
    with get_db() as conn:
        ...
        # 平均賠償金額
        cursor.execute(
            "SELECT AVG(compensation) as avg FROM judgments WHERE compensation > 0"
        )
