...
def test_search(client, mock_db_with_data, monkeypatch):
    ...
    response = client.post('/api/search', json={"query": "配偶與第三人外遇交往長達五年以上，期間多次單獨出國同遊，有出入境紀錄與社群軟體的打卡照片為證。", "top_k": 10, "min_similarity": 0.0})
    assert response.status_code == 200
    data = response.get_json()
    assert 'results' in data
    assert 'stats' in data
    assert data['query'] == "配偶與第三人外遇交往長達五年以上，期間多次單獨出國同遊，有出入境紀錄與社群軟體的打卡照片為證。"
    assert 'search_time_ms' in data


def test_search_invalid_request(client, mock_db_with_data):
    response = client.post('/api/search', json={"query": "短", "top_k": 0, "min_similarity": 0.0})
...
def test_search_fail(client, mock_db_with_data, monkeypatch):
    ...
    response = client.post('/api/search', json={"query": "測試搜尋服務崩潰", "top_k": 10, "min_similarity": 0.0})
