...
def test_search_judgments_parameters(tmp_path, monkeypatch):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE judgments (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, case_number TEXT, court TEXT, date TEXT, compensation INTEGER, facts TEXT, reasoning TEXT, decision TEXT, evidence_types TEXT, vector BLOB)"
        )
        cursor.execute(
            "INSERT INTO judgments (title, case_number, court, date, compensation, facts, reasoning, decision, evidence_types, vector) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                "案由1",
                "案號1",
                "臺灣新北地方法院",
        ...
        cursor.execute(
            "INSERT INTO judgments (title, case_number, court, date, compensation, facts, reasoning, evidence_types, vector) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                "none",
                "none",
                "none",
                ...
                "none",
                "none",
                np.empty(1536, dtype=np.float32).tobytes(),
            ),
        )
    monkeypatch.setattr(
        search_service,
        "get_vector_cache",
        lambda: (np.array([1, 2, 3]), np.random.rand(3, 1536).astype(np.float32)),
    )
    monkeypatch.setattr(
        search_service,
        "query_embeddings",
...
def test_search_judgments_no_result(monkeypatch):
    monkeypatch.setattr(
        search_service, "get_vector_cache", lambda: (np.array([]), np.empty((0, 1536)))
    )
...
def test_search_judgments_stats(tmp_path, monkeypatch):
    ...
    assert response.model_dump()["stats"] == {
        "total": 3,
        "median_compensation": 200000,
        "avg_compensation": 200000.0,
