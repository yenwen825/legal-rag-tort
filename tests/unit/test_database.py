...
def test_get_db_rollback(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO test_table (value) VALUES (?)", ("should be rolled back",)
            )
            raise RuntimeError("test exception")
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT value FROM test_table WHERE value = 'should be rolled back'"
        )
        result = cursor.fetchone()
...
def test_init_db_valid(tmp_path, monkeypatch):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE judgments (id INTEGER PRIMARY KEY AUTOINCREMENT, value TEXT)"
        )
    stats = init_db()
...
def test_get_db_stats_valid(tmp_path, monkeypatch):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE judgments (id INTEGER PRIMARY KEY AUTOINCREMENT, compensation INTEGER)"
        )
