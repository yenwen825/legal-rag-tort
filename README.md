...
- **Data & AI:** [SQLite](https://www.sqlite.org/), [NumPy](https://numpy.org/), [OpenAI API](https://openai.com/) (Embedding: `text-embedding-3-small`, LLM: `gpt-4o-mini`)
...
REDIS_URL=rediss://...
```
*(註：`REDIS_URL` 為選填。若未提供，Rate limit 容量與快取將預設回退至 memory 執行。)*
</details>
...
先於主控台建立 Secrets Manager (`OPENAI_API_KEY`, `REDIS_URL`)，然後以 GCP CLI 發佈：
...
gcloud run deploy legal-rag-tort \
  ...
  --set-secrets=OPENAI_API_KEY=OPENAI_API_KEY:latest,REDIS_URL=REDIS_URL:latest \
...
### Future Roadmap
- **優化 RAG 檢索框架**：
   - 導入 Reranker，修正純 Cosine Similarity 所導致召回判決與 query 爭點不符的狀況。
