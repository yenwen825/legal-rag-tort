    ...
    with get_db() as conn:
        ...
        for judgment in judgments:
            if not judgment["full_text"]:
                logging.warning(
                    f"Full text is empty for judgment {judgment['case_number']}"
                )
                skipped_count += 1
                continue
            ...
            if court != judgment["court"]:
                ...
                cursor.execute(
                    "UPDATE judgments SET court = ? WHERE id = ?",
                    (court, judgment["id"]),
                )
                logging.info(
                    f"Court name for judgment {judgment['case_number']} fixed from {judgment['court']} to {court}"
                )
                fixed_count += 1
        logging.info(
            f"total {total_count} judgments, Court names fixed successfully: {fixed_count} judgments, {skipped_count} judgments skipped because full text is empty or (court name + case number) is already in the database"
        )
