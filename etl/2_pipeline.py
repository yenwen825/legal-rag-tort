...
class GPTExtractedData(BaseModel):
    ...
    reasoning: str = Field(
        ...
        1. 本件爭點（如判決中有「本件之爭點厥為...」等明確列出爭點，務必完整摘要）
        2. 證據能力之審查（如法院有討論證據是否具證據能力、是否可採、證明力強弱等論述）
        3. 法院如何認定被告行為是否構成侵害配偶權（包含事實認定過程、證據取捨理由）
        4. 法院對於兩造主張及抗辯是否採信之理由
        5. 法院如何評估侵害行為的嚴重程度（情節是否重大）
        6. 法院如何判斷賠償金額的適當性
        7. 引用的特別法律條文或判例（僅限法院有特別解釋論述者）
